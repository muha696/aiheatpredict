...
train_size = np.linspace(0.1, 1, 10, endpoint=False)
test = []
mse_list = []
j = 1
for i in train_size:
    test.append((1 - i) * len(X))
    print(f'Итерация {j} Количество train: {(i) * len(X)} Количество test: {(1-i) * len(X)}')
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size= (1 - i), shuffle=False)
    ...
    y_pred_part = model.predict(dval)
    y_test_real = scaler_y.inverse_transform(y_test.reshape(-1, 1)).flatten()
    y_pred_real = scaler_y.inverse_transform(y_pred_part.reshape(-1, 1)).flatten()
    mse = mean_squared_error(y_test_real, y_pred_real)
    mse_list.append(mse)

with pd.ExcelWriter('xgboost_check_mse640.xlsx') as writer:
    pd.DataFrame({
        'test_size': test,
        'train':train_size,
        'train_num': train_size * len(X),
        'mse': mse_list,
    }).to_excel(writer, sheet_name='mse', index=False)
